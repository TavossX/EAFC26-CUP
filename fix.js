const fs = require('fs');
const glob = require('glob'); // Not available by default, let's just use a simple recursive read
...
files.forEach(file => {
  ...
  // Many formats:
  // Format A (same line): bg="brand.surfaceLight" _dark={{ bg: 'brand.surfaceDark' }} borderRadius="4px" borderWidth={1} borderColor="brand.dark" _dark={{ borderColor: 'whiteAlpha.300' }}
  ...
  // Same line fix for `_dark={{ borderColor: ... }} _dark={{ bg: ... }}`
  // Let's just catch all cases where there are 2 _dark attributes on the same tag.
  // Actually, a more robust way is to just replace the whole tag's multiple _dark attributes with one.
  // Since we already did single line, let's see if there are any others.
